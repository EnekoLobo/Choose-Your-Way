...
function JukeBox.VGUI.Pages.AdminRequests:CreatePanel( parent )
	parent.Warning = vgui.Create( "DPanel", parent )
	parent.Warning:Dock( TOP )
	-- parent.Warning:SetTall( 40 )
	parent.Warning:DockPadding( 50, 10, 10, 10 )
	parent.Warning.SongListSize = (#util.TableToJSON(JukeBox.SongList) + 17) / 1024
	parent.Warning.Paint = function( self, w, h )
		-- draw.RoundedBox( 0, 0, h-1, w, 1, JukeBox.Colours.Base 
		if (self.SongListSize/256 > 0.75) then
		...
		draw.RoundedBox( 0, 0, h-5, w*self.SongListSize/256, 5, Color( 0, 0, 0, 150 ) )
		
		JukeBox.VGUI.VGUI:DrawEmblem( 25, h/2, 18, "jukebox/warning.png", Color( 255, 255, 255 ), 0 )
	end

	parent.Warning.Body = vgui.Create( "DLabel", parent.Warning )
	parent.Warning.Body:SetFont( "JukeBox.Font.18" )
	parent.Warning.Body:SetTextColor( Color( 255, 255, 255 ) )
	parent.Warning.Body:SetText( JukeBox.Lang:GetPhrase("#REQUESTS_Warning", math.ceil(parent.Warning.SongListSize), math.ceil(parent.Warning.SongListSize/256 * 100)) )
	parent.Warning.Body:SizeToContents()
	parent.Warning:SetTall( parent.Warning.Body:GetTall() + 20 )
	parent.Warning.Body:Dock( FILL )


	parent.Warning.MoreInfo = vgui.Create( "DButton", parent.Warning )
	parent.Warning.MoreInfo:Dock( RIGHT )
	-- parent.Warning.Right.MoreInfo:DockMargin( 0, 6, 10, 6 )
	parent.Warning.MoreInfo:SetWide( 115 )
	-- parent.Warning.MoreInfo:SetTall( 40 )
	parent.Warning.MoreInfo:SetText( "" )
	parent.Warning.MoreInfo.DoClick = function()
		JukeBox.VGUI:CreatePopup( JukeBox.Lang:GetPhrase( "#POPUP_Warning" ), JukeBox.Lang:GetPhrase( "#REQUESTS_WarningPopup" ) )
	end
	parent.Warning.MoreInfo.Paint = function( self, w, h )
		if JukeBox.VGUI.VGUI:GetHovered( self ) then
			draw.RoundedBox( h/2+1, 0, 0, w, h, JukeBox.Colours.Information )
		else
			draw.RoundedBox( h/2+1, 0, 0, w, h, JukeBox.Colours.Light )
		end
		JukeBox.VGUI.VGUI:DrawEmblem( h/2, h/2, 16, "jukebox/warning.png", Color( 255, 255, 255 ), 0 )
		draw.SimpleText( "More Info", "JukeBox.Font.20", 30, h/2, Color( 255, 255, 255 ), 0, 1 )
	end
