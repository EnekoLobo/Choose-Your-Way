...
force.Author = "Blu-x92 / Luna"
force.Description = "Shroud yourself with the force for 10 seconds"
force.id = "cloak"
force.StartUse = function(ply)
    if ply:lscsGetForce() < 50 or not ply:OnGround() then
        return
    end
    if ply._lscsCloaking then
        return
    end

    ply:lscsTakeForce(50)
    ply:EmitSound("lightsaber/force_leap.wav")
    ply._lscsCloakTime = CurTime() + 10

    -- Aplicar SetNoDraw y SetRenderMode
    ply:SetNoDraw(true)
    ply:SetRenderMode(RENDERMODE_NONE)
    ply._lscsIsCloaked = true

    local steamID64 = ply:SteamID64()
    if timer.Exists("LSCS_CLOAK_" .. steamID64) then
        timer.Destroy("LSCS_CLOAK_" .. steamID64)
    end
    timer.Create("LSCS_CLOAK_" .. steamID64, 0.5, 21, function()
        if not IsValid(ply) then
            timer.Destroy("LSCS_CLOAK_" .. steamID64)
            return
        end
        if not ply:Alive() or ply._lscsCloakTime <= CurTime() then
            timer.Destroy("LSCS_CLOAK_" .. steamID64)

            -- Revertir SetNoDraw y SetRenderMode
            ply:SetNoDraw(false)
            ply:SetRenderMode(RENDERMODE_NORMAL)
            ply._lscsIsCloaked = false
            return
        end

        ply:SetNoTarget(ply:GetActiveWeapon() == ply)
    end)

    return true
end

force.EndUse = function(ply)
    if ply._lscsIsCloaked then
        ply:SetNoDraw(false)
        ply:SetRenderMode(RENDERMODE_NORMAL)
        ply._lscsIsCloaked = false
    end
...
local force = {}
force.PrintName = "Cegar"
force.Author = "Memeti"
force.Description = "Ciegas a un oponente."
force.id = "blind"
force.Cooldown = 60
force.StartUse = function( ply )
    local forceuse = 90
    ...
    for k, v in ipairs(ents.FindInSphere(ply:GetPos(), 200)) do
        ...
        v:ScreenFade(SCREENFADE.OUT, Color(0, 0, 0, 255), 0.2, 4)
    end
...
local force = {}
force.PrintName = "Curacion Grupal"
force.Author = "Apollo"
...
force.StartUse = function( ply )
    ...
    for k,v in ipairs(ents.FindInSphere(ply:GetPos(), 150)) do
        ...
        v:SetHealth(math.min(v:Health() + v:GetMaxHealth()*0.2, v:GetMaxHealth()))
        local effectdata = EffectData()
