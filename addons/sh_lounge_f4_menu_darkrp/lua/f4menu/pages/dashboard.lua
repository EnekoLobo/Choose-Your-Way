	...
	top:Dock(TOP)
		...
		playerinfo.Paint = function(me, w, h)
			...
			image.Paint = function(me, w, h)
				...
				local av = vgui.Create("AvatarImage", image)
		...
		teamrep.Paint = function(me, w, h)
			...
			scrollp.Paint = function(me, w, h)
				...
				for job, num in SortedPairsByValue (showcats and catz or jobz, true) do
					...
					if (!showcats) then
						pnl:SetToolTip(string.format(self.Language.click_to_become_x, team.GetName(job)))
						pnl.DoClick = function()
							RunConsoleCommand("say", "/" .. RPExtraTeams[job].command)
						end
					else
						col = catcolors[job] or self.Style.header
				...
				if (IsValid(richestp)) then
					if (!IsValid(pinfo) or pinfo.m_Player ~= richestp) then
						...
						pinfo.m_Player = richestp
							...
							av:SetPlayer(richestp)
							av:SetSize(32 * scale, 32 * scale)
							av:Dock(LEFT)
