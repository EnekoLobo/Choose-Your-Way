...
local function ShowModelPrompt(self, job, str)
	...
	cancel.Think = function(me)
		...
		buttons:DockMargin(m, 0, m, m)
			...
			prev.DoClick = function(me)
				local prevModel = table.FindPrev(models, curmdl)
			...
			nextb.DoClick = function(me)
				local prevModel = table.FindNext(models, curmdl)
				mdl:SetModel(prevModel)
				mdl.Entity:SetAngles(Angle(0, 45, 0))
				curmdl = prevModel
	...
	jobs.Update = function(me, update, filter)
		...
		for _, cat in pairs (cats) do
			...
			for __, job in pairs (jobz) do
				if (filter and filter ~= "" and !string.find(job.name:lower(), filter, 1, true)) then
					continue end

				local cur = LocalPlayer():Team()
				if ((isnumber(job.NeedToChangeFrom) and cur ~= job.NeedToChangeFrom and update ~= job.NeedToChangeFrom) or 
					(istable(job.NeedToChangeFrom) and !table.HasValue(job.NeedToChangeFrom, cur) and !table.HasValue(job.NeedToChangeFrom, update))) then
					continue end
				...
				local failedcheck = (job.customCheck and !job.customCheck(LocalPlayer()) and !self.KeepFailedCustomCheckJobs) or
									...
									(job.level and LocalPlayer():getDarkRPVar("level") and LocalPlayer():getDarkRPVar("level") < job.level and self.HideHighLevelJobs)
				if (failedcheck) then
					continue end

				local sameteam = job.team == cur
				...
				pnl.DoClick = function(me)
					...
					elseif (isstring(job.model) and self.AlwaysModelPrompt) then
						ShowModelPrompt(self, job, true)
					else
